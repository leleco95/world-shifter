...
function movePlayer(dt)
  ...
  elseif (world.map[player.targetx][player.targety]%3) ~= 0 then
...
function createWorld()
  ...
  for i=1, world.sizex, 1 do
    ...
    for j=1, world.sizey, 1 do
      world.map[i][j] = love.math.random(0,1+world.diff)
    end
  end
  world.map[1][1] = 0
  world.map[1][2] = 0
  ...
  if player.shifts > 0 then
    for i=1, world.sizex, 1 do
      for j=1, world.sizey, 1 do
        ...
        if world.map[i][j] > 1+world.diff then
          world.map[i][j] = 0
        end
      end
    end
    world.map[player.sqmx][player.sqmy]=0
